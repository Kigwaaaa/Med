...
export function EmployeeAuth() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const doctors = await db.from('doctors');
      const { data: doctorData } = await doctors.select();
      const doctor = doctorData.find((d: StaffMember) => d.staff_number === staffNumber.trim());
      ...
      // If not a doctor, try to find a lab technician
      const labTechnicians = await db.from('lab_technicians');
      const { data: labTechData } = await labTechnicians.select();
      const labTech = labTechData.find((l: StaffMember) => l.staff_number === staffNumber.trim());
      
      if (labTech) {
        localStorage.setItem('labTechInfo', JSON.stringify(labTech));
        navigate('/lab');
        return;
      }
